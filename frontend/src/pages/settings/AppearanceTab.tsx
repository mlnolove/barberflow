  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label htmlFor="primary_color" className="field-label">
            Cor primária
          </label>
          <Controller
            control={control}
            ...
            render={({ field }) => (
              <div className="mt-1 flex items-center gap-2">
                <input
                  ...
                  className="h-9 w-11 cursor-pointer rounded-md border border-white/[0.08] disabled:cursor-not-allowed"
                />
                <input
                  disabled={!canEdit}
                  value={field.value}
                  onChange={(e) => field.onChange(e.target.value)}
                  placeholder="#0F172A"
                  className="w-full rounded-md border border-white/[0.08] bg-ink-800 px-3 py-2 text-sm uppercase text-white disabled:opacity-60"
                />
              </div>
            )}
          />
          {errors.primary_color && (
            <p className="field-error">{errors.primary_color.message}</p>
          )}
        </div>

        <div>
          <label htmlFor="secondary_color" className="field-label">
            Cor secundária
          </label>
          <Controller
            control={control}
          ...
          {errors.secondary_color && (
            <p className="field-error">{errors.secondary_color.message}</p>
          )}
        </div>
      </div>

      <div>
        <label htmlFor="logo_url" className="field-label">

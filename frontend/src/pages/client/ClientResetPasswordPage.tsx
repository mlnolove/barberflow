...
import { confirmClientPasswordReset } from "@/api/clientAuth";
...
export function ClientResetPasswordPage() {
  ...
  async function onSubmit(values: FormValues) {
    ...
    setServerError(null);
    try {
      await confirmClientPasswordReset(token, values.password);
      setDone(true);
    } catch (error: unknown) {
  ...
  if (!token) {
    return (
      <div className="flex h-screen flex-col bg-ink-950">
        <ClientTopBar onBack={() => navigate("/c/entrar")} />
        <div className="flex flex-1 flex-col items-center justify-center gap-3 px-8 text-center">
          ...
          <AuthButton
            type="button"
            onClick={() => navigate("/c/esqueci-senha")}
            className="mt-2"
          >
            Pedir novo link
          </AuthButton>
    ...
    return (
      <div className="flex h-screen flex-col bg-ink-950">
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-4 px-8 text-center">
          ...
          <AuthButton type="button" onClick={() => navigate("/c/entrar")} className="mt-2">
            Ir para o login
          </AuthButton>
        </div>
      </div>
